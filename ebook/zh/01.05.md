...
public static int getPalindromeLength(String str) {
        ...
        // rl[i]表示以i为中心的回文的最大半径，i至少为1，即该字符本身
        int[] rl = new int[newStr.length()];
        ...
        for (int i = 0; i < newStr.length(); i++) {
            ...
            int r = 1;
            if (i < maxRight) {
                //假设i的对称位置是j 则 j = pos-(i-pos) = 2* pos -i
                r = Math.min(rl[2 * pos - i], maxRight - i);
            }
            // 2.2.尝试更大的半径
            while (i - r >= 0 &&
                    i + r < newStr.length()
